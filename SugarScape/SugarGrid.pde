...
class SugarGrid {
   ...
   *
   */
  public SugarGrid(int w, int h, int sideLength, GrowthRule g,FertilityRule frule,ReplacementRule rrule) {
    this.howWide = w;
    this.howHigh = h;
  ...
  public void update() {
    ...
    for (int i = 0; i < howWide; i++) {
      for (int j = 0; j < howHigh; j++) {
        ...
        LinkedList<Square> local = generateVision(i,j,1);
        LinkedList<Agent> available = new LinkedList<Agent>();
        for(Square sq : local){
          if(sq.getAgent() != null) {
            available.add(sq.getAgent());
          //  if(fr.canBreed(a,sq.getAgent(),local)){
            //  LinkedList<Square> otherLocal = generateVision(sq.getX(),sq.getY(),1);
            //  fr.breed(a,sq.getAgent(),local, otherLocal);
          //  }
          }
        }
        for(Agent o : available){
          if(fr.canBreed(a,o,local)){
            LinkedList<Square> otherLocal = generateVision(o.getSquare().getX(),o.getSquare().getY(),1);
            fr.breed(a,o,local,otherLocal);
          }
        }
  ...
  */
  ...
  /* sets sugarlevel of an agent to 0, then calls replacementrule's replaceThisOne on agent to change age, calls fertitilty rule's isFertile method on agent to remove them from storage.
  */
...
class SugarGridTester {
  void test() {
    ...
    FertilityRule fr = null;
    ReplacementRule rr = null;
    int w = 20;
    int h = 20;
    int sideLength = 15;

    // constructor, accessors
    SugarGrid sg = new SugarGrid(w, h, sideLength, gr,fr,rr);
