...
class ReplacementRule {
  ...
  private int maxAge;
  AgentFactory fac;
  HashMap<Agent,Integer> agentAge;  
  ...
  */
  public ReplacementRule(int minAge, int maxAge, AgentFactory fac) {
    ...
    this.maxAge = maxAge;
    this.fac = fac;
    this.agentAge = new HashMap<Agent,Integer>();
  }
  ...
  */
  public boolean replaceThisOne(Agent a) {
    if (!agentAge.containsKey(a)) {
      Random r = new Random();
      agentAge.put(a,(minAge + r.nextInt(maxAge+1 - minAge)));
    }
      if (a.getAge() > agentAge.get(a)) {
        a.setAge(maxAge+1);
        return true;
      }
    return false;
...
class ReplacementRuleTester {
  public void test() {
    ...
    int ls = rr.agentAge.get(a);
    assert(ls <= maxAge && ls >= minAge);
    a.step();
    rr.agentAge.remove(a);
    rr.agentAge.put(a,a.getAge()-1);
